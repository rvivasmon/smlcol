...
<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col">
                <h1 class="m-0">Pitchs Disponibles</h1>
                            ...
                            <div class="table-responsive">
                                <table id="table_usuarios" class="table table-striped table-hover table-bordered">
                                    ...
                                    <tbody>
                                        <?php
                                        $contador = 0;
                                        $query = $pdo->prepare('SELECT * FROM tabla_pitch WHERE pitch IS NOT NULL AND pitch != "" ORDER BY pitch');
                                        $query->execute();
                                        $pitches = $query->fetchAll(PDO::FETCH_ASSOC);
                                        foreach ($pitches as $pitch){
                                            $id = $pitch['id'];
                                            $nombre_pitch = $pitch['pitch'];
                                            $habilitado = $pitch['habilitar_pitch'];
                                            $contador = $contador + 1;
                                        ?>
                                            <tr>
                                                <td><?php echo $contador; ?></td>
                                                <td><?php echo $nombre_pitch; ?></td>
                                                <td>
