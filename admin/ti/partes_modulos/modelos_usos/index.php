...
<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col">
                <h1 class="m-0">Modelos Disponibles</h1>
                            ...
                            <div class="table-responsive">
                                <table id="table_usuarios" class="table table-striped table-hover table-bordered">
                                    ...
                                    <tbody>
                                        <?php
                                        $contador = 0;
                                        $query = $pdo->prepare('SELECT * FROM t_tipo_producto WHERE modelo_modulo IS NOT NULL AND modelo_modulo != "" ORDER BY modelo_modulo');
                                        $query->execute();
                                        $nitses = $query->fetchAll(PDO::FETCH_ASSOC);
                                        foreach ($nitses as $nits){
                                            $id = $nits['id'];
                                            $nombre_nits = $nits['modelo_modulo'];
                                            $habilitado = $nits['habilitar_modelo_modulo'];
                                            $contador = $contador + 1;
                                        ?>
                                            <tr>
                                                <td><?php echo $contador; ?></td>
                                                <td><?php echo $nombre_nits; ?></td>
                                                <td>
