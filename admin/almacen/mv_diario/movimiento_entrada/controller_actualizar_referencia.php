...
if (isset($_FILES['archivo_adjunto']) && $_FILES['archivo_adjunto']['error'] === UPLOAD_ERR_OK) {
    ...
    // Si no hay archivo o es nulo, solo actualizar la base de datos sin archivo
    $query = $pdo->prepare("UPDATE producto_modulo_creado SET referencia = :referencia WHERE id = :moduloId");
    $query->bindParam(':referencia', $referencia);
    $query->bindParam(':moduloId', $moduloId);
    $query->execute();
}
