...
<div class="content-wrapper">
                                        ...
                                        <div class="card">
                                            <div class="card-body">
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-3">
                                                        <div class="form-group">
                                                            <label for="sub_almacen">Sub Almacén destino</label>
    ...
    <script>
        ...
        <div class="modal fade" id="modalMovimiento" tabindex="-1" aria-labelledby="modalMovimientoLabel" aria-hidden="true" role="dialog">
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalMovimientoLabel">Mercancía a Validar</h5>
    ...
    <script>
    $(document).ready(function () {
        cargarMovimientos(); // Cargar datos al abrir la página

        // Función para obtener y mostrar los movimientos en la tabla
        function cargarMovimientos() {
            $.ajax({
                ...
                dataType: "json",
                beforeSend: function () {
                    $("#tablaMovimientos").html('<tr><td colspan="8">Cargando...</td></tr>'); // Loader
                },
                success: function (data) {
                    if (data.length > 0) {
                        let tabla = data.map(item => `
                            <tr>
                                ...
                                <td><input type="number" class="form-control form-control-sm cantidad" data-id="${item.id_movimiento_admon}" data-original="${item.cantidad_entrada}" value="${item.cantidad_entrada}"></td>
                                ...
                                <td><input type="text" class="form-control form-control-sm observaciones" data-id="${item.id_movimiento_admon}" data-original="${item.observaciones ?? ''}" value="${item.observaciones ?? ''}"></td>
                                <td><button class="btn btn-success btn-sm procesar" data-id="${item.id_movimiento_admon}">Procesar</button></td>
                            </tr>
                        `).join("");
                        ...
                        $("#tablaMovimientos").html('<tr><td colspan="8">No hay movimientos disponibles.</td></tr>');
                    }
                },
                error: function (xhr, status, error) {
                    console.error("Error al obtener los datos:", error);
                    $("#tablaMovimientos").html('<tr><td colspan="8">Error al cargar los datos.</td></tr>');
                }
            });
        }
        ...
        $(document).on("click", ".procesar", function () {
            ...
            let id = $(this).data("id");
            let cantidadActual = row.find(".cantidad").val().trim();
            let observacionesActual = row.find(".observaciones").val().trim();

            // Obtener los valores originales
            let cantidadOriginal = row.find(".cantidad").data("original");
            let observacionesOriginal = row.find(".observaciones").data("original");

            // Si no hay cambios, no enviar nada
            if (cantidadActual == cantidadOriginal && observacionesActual == observacionesOriginal) {
            ...
            $.ajax({
                ...
                method: "POST",
                data: { id, cantidad: cantidadActual, observaciones: observacionesActual },
                success: function (response) {
                    console.log("Respuesta del servidor:", response);
                    if (response.mensaje) {
        alert("Registro procesado correctamente.");
        row.remove(); // Eliminar fila del frontend
        
        // Si ya no hay filas, cerrar el modal
        if ($("#tablaMovimientos tr").length === 0) {
            $("#modalMovimiento").modal("hide");
        }
    } else {
        alert("Hubo un problema: " + (response.error || "Error desconocido"));
    }
},
                error: function (xhr, status, error) {
